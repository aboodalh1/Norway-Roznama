...
class ArticlesRepoImpl implements ArticlesRepo {
  ...
  Future<Either<Failure, Response>> getArticles(
    ...
    try {
      var response = await dioHelper.getData(
          ...
          query: title != null && sectionId != null
              ? {"section_id": sectionId, "title": title, 'perPage': 5}
              : sectionId != null
                  ? {"section_id": sectionId, "perPage": 5}
                  : title != null
                      ? {"title": title, "perPage": 5}
                      : {"perPage": 5});
      return right(response);
    } catch (e) {
      if (e is DioException) {
  ...
  @override
  Future<Either<Failure, Response>> searchForArticle(
  ...
  Future<Either<Failure, Response>> getMoreArticles({required path}) async {
    try {
      var response = await dioHelper
          .getData(link: path, endPoint: 'api/v1/blogs', query: {'perPage': 5});
      return right(response);
    } catch (e) {
      if (e is DioException) {
        return left(ServerFailure.fromDioError(e));
  ...
  Future<Either<Failure, Response>> getSections({required num number}) async {
    try {
      var response = await dioHelper
          .getData(endPoint: 'api/v1/sections', query: {'perPage': 10});
      return right(response);
    } catch (e) {
      if (e is DioException) {
        return left(ServerFailure.fromDioError(e));
